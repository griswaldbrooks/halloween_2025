...
void setup() {
  ...
  Serial.println(F("Hatching Egg Spider - Animation Tester"));
  Serial.println(F("========================================"));

  // Initialize trigger pin (optional - for testing trigger)
  ...
  Serial.print(F("Available animations: "));
  Serial.println(ANIMATION_COUNT);

  // Move to resting position
  moveToResting();

  Serial.println(F("Ready! Use serial commands to test animations."));
  Serial.println();
  printHelp();
}
